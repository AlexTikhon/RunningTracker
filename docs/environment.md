...
Node 24 is an LTS line and is supported by the selected Vite and NestJS runtime versions. The NestJS schematics have a higher patch floor than the installed Node version, so this repository does not depend on global generators; its build uses local `tsc`/`tsx` commands.

Official references checked during P00:

- Node.js release schedule: https://nodejs.org/en/about/previous-releases
- Vite Node.js requirements: https://vite.dev/guide/
- NestJS runtime and CLI requirements: https://docs.nestjs.com/migration-guide
