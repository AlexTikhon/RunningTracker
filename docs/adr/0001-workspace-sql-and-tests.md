...
The SDD calls for one modular monolith, a React client, NestJS API, PostgreSQL/PostGIS, explicit SQL, and later RLS/security tests. The project is new and has no repository conventions to preserve.
...
2. Use Node 24 LTS, strict TypeScript, React/Vite, and NestJS. Pin dependency resolution in `package-lock.json`; pin the PostGIS container by version and digest.
...
- `packages/contracts` must remain independent from NestJS, `pg`, secrets, and infrastructure code.
