...
3. `withTenantTransaction` validates canonical UUIDs, checks out one client, begins a transaction, sets `app.user_id` and `app.org_id` with `set_config(..., true)`, and gives that same client to the callback. The helper exclusively owns the outer `BEGIN`/`COMMIT`/`ROLLBACK` boundary and client release; the callback may use savepoints but must not finish the outer transaction or release the client. A result is returned only after PostgreSQL reports `command = 'COMMIT'`. `command = 'ROLLBACK'` is a confirmed rollback with a known unsuccessful outcome; a COMMIT execution failure or unrecognized command is an unknown outcome and destroys the connection. Rollback failure does not replace the callback error and also destroys the connection.
